  ...
  return (
    ...
    >
      ...
      <div className="relative flex basis-0 justify-end space-x-6 sm:space-x-8 md:flex-grow">
        ...
        <Link href="https://github.com/coral-xyz/anchor-by-example" className="group">
...
export function Layout({ children, title, navigation, tableOfContents }: LayoutProps) {
  ...
  function isActive(section: TableOfContentsItem): boolean {
      ...
      <div className="relative mx-auto flex max-w-8xl justify-center sm:px-2 lg:px-8 xl:px-12">
        ...
        <div className="min-w-0 max-w-2xl flex-auto px-4 py-16 lg:max-w-none lg:pr-0 lg:pl-8 xl:px-16">
          ...
          <article>
            ...
            <Prose className="prose-slate">{children}</Prose>
          </article>
              ...
              <>
                ...
                <ul className="mt-4 space-y-3 text-sm">
                  {tableOfContents.map((section) => (
                    ...
                    <li key={section.id}>
                      <h3>
                        <Link href={`#${section.id}`} className={clsx(
                            isActive(section)
                              ? 'text-sky-500'
                        ...
                        <ul className="mt-2 space-y-3 pl-5 text-slate-500 dark:text-slate-400">
                          {section.children.map((subSection) => (
                            <li key={subSection.id}>
                              <Link href={`#${subSection.id}`} className={
                                  isActive(subSection)
                                    ? 'text-sky-500'
...
function useTableOfContents(tableOfContents: TableOfContentsItem[]) {
  ...
  let getHeadings = useCallback(() => {
    function* traverse(node: TableOfContentsItem | TableOfContentsItem[]): Generator<{ id: string; top: number }> {
